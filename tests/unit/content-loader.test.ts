...
describe('content loader (real repository content)', () => {
  it('loads the three hundred and twenty-four lessons with unique ids', () => {
    const lessons = getAllLessons();
    expect(lessons).toHaveLength(324);
    expect(new Set(lessons.map((lesson) => lesson.lesson_id)).size).toBe(324);
  });

  it('loads the three hundred and twenty-seven printable resources and resolves every printable reference', () => {
    expect(getAllResources()).toHaveLength(327);
  ...
  it('renders the approved curriculum in production mode (ADR-015 go-live)', () => {
    ...
    expect(getPublicLessons('production')).toHaveLength(324);
    expect(getPublicLessons('preview')).toHaveLength(324);
  });
