...
describe('content loader (real repository content)', () => {
  it('loads the eighty-three lessons with unique ids', () => {
    const lessons = getAllLessons();
    expect(lessons).toHaveLength(83);
    expect(new Set(lessons.map((lesson) => lesson.lesson_id)).size).toBe(83);
  });

  it('loads the eighty-nine printable resources and resolves every printable reference', () => {
    expect(getAllResources()).toHaveLength(89);
  ...
  it('hides draft seed lessons from production mode (governance boundary)', () => {
    ...
    expect(getPublicLessons('preview')).toHaveLength(83);
  });

  it('groups lessons by segment in sequence order for preview builds', () => {
    const kids = getPublicLessonsBySegment('Kids', 'preview');
    expect(kids.map((lesson) => lesson.lesson_id)).toEqual(['K1-L01', 'K1-L02', 'K1-L03', 'K1-L04', 'K1-L05', 'K1-L06', 'K1-L07', 'K1-L08', 'K1-L09', 'K1-L10', 'K1-L11', 'K1-L12', 'K1-L13', 'K5-L13', 'K1-L14', 'K1-L15', 'K1-L16', 'K1-L17', 'K1-L18', 'K1-L19', 'K1-L20', 'K1-L21', 'K1-L22', 'K1-L23', 'K1-L24', 'K1-L25', 'K1-L26']);
