...
describe('content loader (real repository content)', () => {
  it('loads the two hundred and eighty-eight lessons with unique ids', () => {
    const lessons = getAllLessons();
    expect(lessons).toHaveLength(288);
    expect(new Set(lessons.map((lesson) => lesson.lesson_id)).size).toBe(288);
  });

  it('loads the two hundred and ninety-one printable resources and resolves every printable reference', () => {
    expect(getAllResources()).toHaveLength(291);
  ...
  it('renders the approved curriculum in production mode (ADR-015 go-live)', () => {
    ...
    expect(getPublicLessons('production')).toHaveLength(288);
    expect(getPublicLessons('preview')).toHaveLength(288);
  });
