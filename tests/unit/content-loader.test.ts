...
describe('content loader (real repository content)', () => {
  it('loads the forty-eight lessons with unique ids', () => {
    const lessons = getAllLessons();
    expect(lessons).toHaveLength(48);
    expect(new Set(lessons.map((lesson) => lesson.lesson_id)).size).toBe(48);
  });

  it('loads the fifty-three printable resources and resolves every printable reference', () => {
    expect(getAllResources()).toHaveLength(53);
  ...
  it('hides draft seed lessons from production mode (governance boundary)', () => {
    ...
    expect(getPublicLessons('preview')).toHaveLength(48);
  });

  it('groups lessons by segment in sequence order for preview builds', () => {
    const kids = getPublicLessonsBySegment('Kids', 'preview');
    expect(kids.map((lesson) => lesson.lesson_id)).toEqual(['K1-L01', 'K1-L02', 'K1-L03', 'K1-L04', 'K1-L05', 'K1-L06', 'K1-L07', 'K1-L08', 'K1-L09', 'K1-L10', 'K1-L11', 'K1-L12', 'K1-L13', 'K5-L13', 'K1-L14', 'K1-L19']);
